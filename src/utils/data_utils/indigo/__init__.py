...
from src.utils.data_utils.constants import RGROUP_SYMBOLS,ELEMENTS,SUBSTITUTIONS,COLORS
from src.utils import get_pylogger,normalize_nodes
...
class Indigo(object):
    ...
    def __init__(self, args,g_args,path=None):
        self.dearmotize_prob=args.dearmotize_prob
        self.comment_prob=args.comment_prob
        self.hygrogen_prob=args.hygrogen_prob
        self.rgroup_prob=args.rgroup_prob
        self.condensed_prob=args.condensed_prob
        self.functional_group_prob=args.functional_group_prob
        self.indigo_color_prob=args.indigo_color_prob
        self.g_args=g_args
    ...
    def init_default_option(self):
        """initial default option for indigo dataset
        """
        self.setOption('render-output-format', 'png')
        self.setOption('render-background-color', '1,1,1')
        self.setOption('render-stereo-style', 'none')
        self.setOption('render-label-mode', 'hetero')
        self.setOption('render-font-family', 'Arial')
        
    def reset_option(self):
        """modify default option for indigo dataset
        """
        thickness = random.uniform(0.5, 2)  # limit the sum of the following two parameters to be smaller than 4
        self.setOption('render-relative-thickness', thickness)
        self.setOption('render-bond-line-width', random.uniform(1, 4 - thickness))
        if random.random() < 0.5:
            self.setOption('render-font-family', random.choice(['Arial', 'Times', 'Courier', 'Helvetica']))
        self.setOption('render-label-mode', random.choice(['hetero', 'terminal-hetero']))
        self.setOption('render-implicit-hydrogens-visible', random.choice([True, False]))
        if random.random() < 0.1:
            self.setOption('render-stereo-style', 'old')
        if random.random() < 0.2:
            self.setOption('render-atom-ids-visible', True)
    
    def add_comment(self):
        if random.random() < self.comment_prob:
            self.setOption('render-comment', str(random.randint(1, 20)) + random.choice(string.ascii_letters))
            self.setOption('render-comment-font-size', random.randint(40, 60))
            self.setOption('render-comment-alignment', random.choice([0, 0.5, 1]))
            self.setOption('render-comment-position', random.choice(['top', 'bottom']))
            self.setOption('render-comment-offset', random.randint(2, 30))
            
    def add_explicit_hydrogen(self, mol):
        atoms = []
        for atom in mol.iterateAtoms():
            try:
                ...
                continue
        if len(atoms) > 0 and random.random() < self.hygrogen_prob:
    ...
    def add_rgroup(self, mol, smiles):
        atoms = []
        for atom in mol.iterateAtoms():
            try:
                hs = atom.countImplicitHydrogens()
        ...
        if len(atoms) > 0 and '*' not in smiles:
            if random.random() < self.rgroup_prob:
    ...
    def get_rand_num(self):
        """Code comments by Ma Rong
        """
        # 返回 空 或 2到9 或12 到19，为啥是这几种数字呀，为啥 digit 1 就不能在个位出现了
        if random.random() < 0.9:
            if random.random() < 0.8: # 90% * 80% 概率 返回 空
                return ''
            else: # 90% * 20% 概率 返回 2到9
                return str(random.randint(2, 9))
        else: # 10% 概率 返回 12到19
            return '1' + str(random.randint(2, 9))
    
    def get_rand_symb(self,depth):
        ...
        if random.random() < 0.1:
            symb = f"({self.gen_rand_condensed(depth+1)})"
            # symb = f"({symb})"
        return symb

    def gen_rand_condensed(self,depth=0):
        tokens = []
        for i in range(5):
            if i >= 1 and random.random() < 0.8:
                break
            tokens.append(self.get_rand_symb(depth+1))
            tokens.append(self.get_rand_num())
        return "".join(tokens)
    
    def add_rand_condensed(self, mol):
        atoms = []
        for atom in mol.iterateAtoms():
            try:
                hs = atom.countImplicitHydrogens()
                ...
                continue
        if len(atoms) > 0 and random.random() < self.condensed_prob:
            atom = random.choice(atoms)
            symbol = self.gen_rand_condensed()
            r = mol.addAtom(symbol)
            r.addBond(atom, 1)
        return mol
    
    def add_functional_group(self, mol, debug=False):
        if random.random() > self.functional_group_prob:
            return mol
        ...
        for sub in substitutions:
            query = self.loadSmarts(sub.smarts)
            matcher = self.substructureMatcher(mol)
    ...
    def add_color(self, mol):
        if random.random() < self.indigo_color_prob:
            self.setOption('render-coloring', True)
        if random.random() < self.indigo_color_prob:
            self.setOption('render-base-color', random.choice(list(COLORS.values())))
        if random.random() < self.indigo_color_prob:
            if random.random() < 0.5:
                self.setOption('render-highlight-color-enabled', True)
                self.setOption('render-highlight-color', random.choice(list(COLORS.values())))
            if random.random() < 0.5:
                self.setOption('render-highlight-thickness-enabled', True)
            for atom in mol.iterateAtoms():
                if random.random() < 0.1:
                    atom.highlight()
        return mol
    
    def generate_output_smiles(self, mol):
        """Code comments by Ma Rong
        """
        # TODO: if using mol.canonicalSmiles(), explicit H will be removed
        smiles = mol.smiles()
        mol = self.loadMolecule(smiles)
        # 到这里，smiles 和 mol 都是没问题的
        if '*' in smiles: # smiles 中的 * 替换 为 [symb]，这之后，smiles 就未必可解析了
            ...
            for i, c in enumerate(part_a):
                if c != '*':
                    ...
                    cnt += 1
            return mol, output
        else: # 对于没有 * 的 smiles, 只取第一个空格前的部分
            if ' ' in smiles:
                # special cases with extension
                smiles = smiles.split(' ')[0]
            return mol, smiles
    
    def get_graph(self,mol, image,pseudo_coords=False):
        mol.layout()
        coords, symbols = [], []
        index_map = {}
        atoms = [atom for atom in mol.iterateAtoms()]
        if self.g_args.shuffle_nodes:
            random.shuffle(atoms)
        # Added by Ma Rong: Start
        # valence degree num_Hs_total num_Hs_implicit
        # is_R_site = [] # 是否Rsite [True,False]
        # is_pseudo_atom = [] # 是否伪原子 [True,False]
        # radical_electrons = [] # 自由基电子的数量，Rsite无法获得这个信息（0）。default=0
        # num_Hs_total = [] # 连接的H原子的总数，Rsite无法获得这个信息（0）。
        # num_Hs_implicit = [] # 连接的隐式H原子的数量。Rsite无法获得这个信息（0）。
        # valence = [] # 化合价。Rsite无法获得这个信息（None）。 >0
        # isotope = [] # 同位素。未定义或不明确时，值为0。default=0
        # degree = [] # 显式的度。
        # charge = [] # 电荷量。 default=0
        # single_allowed_R_group = [] # 仅 R基团 可以查询 default=0
        # Added by Ma Rong: Done
        for i, atom in enumerate(atoms):
            if pseudo_coords:
                ...
                x, y = atom.coords()
            coords.append([x, y])
            symbols.append(atom.symbol()) # - symbol— 返回包含原子符号的字符串。它要么是元素周期表中的符号（“C”、“Na”），要么是伪原子标签（“Res”），要么是 R 位标记（“R1”）。
            index_map[atom.index()] = i
            # Added by Ma Rong: Start
            # isotope.append(atom.isotope())
            # degree.append(atom.degree())
            # charge.append(atom.charge())
            # i_is_pseudo_atom = atom.isPseudoatom()
            # is_pseudo_atom.append(i_is_pseudo_atom)
            # i_is_R_site = atom.isRSite()
            # is_R_site.append(i_is_R_site)
            # if i_is_R_site or i_is_pseudo_atom:
            #     radical_electrons.append(0)
            #     num_Hs_total.append(0)
            #     num_Hs_implicit.append(0)
            #     valence.append(1)
            #     if i_is_R_site:
            #         single_allowed_R_group.append(atom.singleAllowedRGroup())
            #     else:
            #         single_allowed_R_group.append(0)
            # else:
            #     radical_electrons.append(atom.radicalElectrons())
            #     num_Hs_total.append(atom.countHydrogens())
            #     num_Hs_implicit.append(atom.countImplicitHydrogens())
            #     valence.append(atom.valence())
            #     single_allowed_R_group.append(0)
            # Added by Ma Rong: Done
        if pseudo_coords:
        ...
        for bond in mol.iterateBonds():
            ...
            edge_list.append([s,t,bond.bondOrder()])
        ...
        graph = {
            ...
            # # Added by Ma Rong: Start
            # 'is_R_site': is_R_site, 
            # 'is_pseudo_atom': is_pseudo_atom, 
            # 'radical_electrons': radical_electrons, 
            # 'num_Hs_total': num_Hs_total, 
            # 'num_Hs_implicit': num_Hs_implicit, 
            # 'valence': valence, 
            # 'isotope': isotope, 
            # 'degree': degree, 
            # 'charge': charge, 
            # 'single_allowed_R_group': single_allowed_R_group
            # # Added by Ma Rong: Done
        }
        # Added by Ma Rong: Start
        # num_atoms = len(symbols)
        # assert len(coords) == num_atoms and len(is_R_site) == num_atoms and len(is_pseudo_atom) == num_atoms and len(radical_electrons) == num_atoms and len(num_Hs_total) == num_atoms and len(num_Hs_implicit) == num_atoms and len(valence) == num_atoms and len(isotope) == num_atoms and len(degree) == num_atoms and len(charge) == num_atoms and len(single_allowed_R_group) == num_atoms
        return graph
    ...
    def all_bonds_explicit_func(smiles):
        ...
        #print(tokens)
        # 在原子前/表示环结尾的数字前，
        # 判断前一位如果不是键（-:=#, /和\\（这是带了转义符号的）代表=两旁的单键是同侧还是异侧，/ \\是同侧即Z构型，/ /是异侧即E构型）
        # 则应当在其前一位加一个键，被省略的一定是-:
        # 到底是什么应该看前面连着这个 原子/表示环结尾的数字代表的原子（即表示这个环对应开头的原子） 是大写还是小写
        # 开始
        stack_tokens = []
        atom_ptr = 0
        last_token = ''
        rings_start_atom = {}
        for token in tokens:
            if token == '.': # 句点无条件清空栈（句点表示分离的结构）
                stack_tokens = []
                new_smiles += token
            elif token == '(': # 左括号无条件进栈
                stack_tokens.append(token)
                new_smiles += token
            elif token == ')': # 右括号，无条件出栈至出掉一个左括号
                first_token = stack_tokens[0]
                while (len(stack_tokens) > 0) and (stack_tokens.pop() != '('):
                    pass
                if (len(stack_tokens) <= 0) and (first_token != '('): # 栈空了，都没有见到左括号，则报错
                    print('[Error Info] the rule of pretokenizer is unexpected for smiles=' + smiles)
                    assert 0
                new_smiles += token
            elif token.isalpha() or token.startswith("[") or token == '*': # 这些是原子。原子无条件进栈
                if not(last_token in ['-', ':', '=', '#', '\\', '/']): # 如果没有键
                    last_atom = '' # 找上一个是原子的栈顶元素，而非栈顶元素，因为有可能是左括号
                    for last_atom_pos in range(len(stack_tokens) - 1, -1, -1):
                        candidate_last_atom = stack_tokens[last_atom_pos]
                        if candidate_last_atom.isalpha() or candidate_last_atom.startswith("[") or candidate_last_atom == '*': # 这些是原子。
                            last_atom = candidate_last_atom
                            break
                    if last_atom == '': # 没有上一个原子，则没有键，而非省略了键
                        pass
                    else: # 有上一个原子，则是省略了键的情况
                        ...
                        if (last_atom in arom_set) and (this_atom in arom_set): # 若上一个原子和这一个原子都是芳香原子，则省略的是芳香键。说明：两个芳香原子之间未必是芳香键，但是两个芳香原子之间的单键并不会被省略，所以这种情况不会进入到这个条件判断
                            #print(stack_tokens, candidate_last_atom, last_atom, this_atom)
                            new_smiles += ':'
                        else: # 若上一个原子不是芳香原子，则省略的是单键
                            new_smiles += '-'
                new_smiles += token
                stack_tokens.append(token)
            elif (len(token) == 1 and '0' <= token <= '9') or (len(token) == 3 and token.startswith('%') and '10' <= token[1:] <= '99'): # 代表环的数字。不压栈，不出栈。
                ring_atom = '' # 找上一个是原子的栈顶元素，而非栈顶元素，因为有可能是左括号
                for last_atom_pos in range(len(stack_tokens) - 1, -1, -1):
                    candidate_last_atom = stack_tokens[last_atom_pos]
                    if candidate_last_atom.isalpha() or candidate_last_atom.startswith("[") or candidate_last_atom == '*': # 这些是原子。
                        ring_atom = candidate_last_atom
                        break
                if token in rings_start_atom: # 若在环字典中已有同样的数字，则是此环的末尾（清空这种环的符号所对应的原子, 如果是省略了键则加上）
                    # 清空这种环的符号所对应的原子
                    last_atom = rings_start_atom.pop(token)
                    if not(last_token in ['-', ':', '=', '#', '\\', '/']): # 如果没有键，环的末尾前没有键，就是省略了键
                        last_atom = ''.join(re.split(r'[^A-Za-z]', last_atom.lstrip('[').rstrip(']').split('@')[0]))
    ...
    def generate_indigo_image(self,renderer,smiles,all_bonds_explicit=False):
        self.init_default_option()
        if not self.g_args.default_option:
            self.reset_option()
        try:
            mol = self.loadMolecule(smiles)
            if self.g_args.mol_augment:
                if random.random() < self.dearmotize_prob:
                    mol.dearomatize()
                else:
                    mol.aromatize()
                smiles = mol.canonicalSmiles()
                self.add_comment()
                mol = self.add_explicit_hydrogen(mol)
                mol = self.add_rgroup(mol, smiles)
                if self.g_args.include_condensed:
                    mol = self.add_rand_condensed(mol)
                mol = self.add_functional_group( mol, self.g_args.debug)
                mol = self.add_color(mol)
                mol, smiles = self.generate_output_smiles(mol)

            buf = renderer.renderToBuffer(mol)
            img = cv2.imdecode(np.asarray(bytearray(buf), dtype=np.uint8), 1) 
            if all_bonds_explicit:
                smiles = self.all_bonds_explicit_func(smiles)
            success = True
        except Exception as e:
            log.error(f'Exception: {smiles} {e}')
            if self.g_args.debug:
                log.error(f'Exception: {smiles} {e}')
                raise Exception
            img = None
            mol = None
            success = False
        return img, smiles, mol, success